...
case $1 in
  ...
  --lint)
    pylint -j0 --disable=C0209,C0116,C0115,E0401,R0801,C0103,R0903 `find {src,tests}|grep .py$|xargs`
    ;;

  --test)
    export PYTHONPATH=$PYTHONPATH:src; coverage run --source=./src,./tests -m pytest
    coverage report
